...
class Trainer_DDP:
    def __init__(self, config):
        ...
        self.log_func(f"Start DDP on rank {self.local_rank}, cuda {self.device}.")
    ...
    def train(self):
        ...
        if self.local_rank == 0:
            ...
            self.logger.debug(summary(self.model, input_data=[x, cell_idx, output_idx], verbose=0))
        ...
        for epoch in range(num_epochs):
            ...
            if ((epoch+1) % num_valid_epochs == 0):
                self.log_func(f'train_dataset')
                self.valid_epoch(self.train_loader)
                self.log_func(f'valid_dataset')
                self.valid_epoch(self.valid_loader)
        ...
        self.log_func(f'local_rank = {self.local_rank:1}, finish training.')
    ...
    def train_epoch(self, train_loader=None):
        ...
        self.log_func(f'local_rank = {self.local_rank:1}, epoch = {self.epoch:3}, train_loss = {train_loss:.6f}')
    ...
    def valid_epoch(self, valid_loader=None):
        ...
        for task_idx, task_name in enumerate(self.task_names):
            ...
            for metric_func in self.metric_func_list:
                ...
                self.log_func(f'local_rank = {self.local_rank:1}, epoch = {self.epoch:3}, '
                              f'task_name = {task_name:16}, valid_model, {type(metric_func).__name__} = {score:.6f}')
