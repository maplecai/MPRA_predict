...
class Trainer_DDP:
    def __init__(self, config):
        ...
        if config.get('random_data_split', False) is True:
            total_dataset = utils.init_obj(datasets, config['train_dataset'])
            indices_list = np.arange(len(total_dataset))
            train_indices, valid_indices, test_indices = utils.split_dataset(
                indices_list, train_valid_test_ratio=config['train_valid_test_ratio'])
        else:
            train_indices, valid_indices, test_indices = None, None, None

        self.train_dataset = utils.init_obj(datasets, config['train_dataset'], selected_indices=train_indices)
        self.valid_dataset = utils.init_obj(datasets, config['valid_dataset'], selected_indices=valid_indices)
        self.train_sampler = DistributedSampler(self.train_dataset)
        self.valid_sampler = DistributedSampler(self.valid_dataset)
        self.train_loader = utils.init_obj(
            torch.utils.data, config['data_loader'], dataset=self.train_dataset, sampler=self.train_sampler, shuffle=False)
        self.valid_loader = utils.init_obj(
            torch.utils.data, config['data_loader'], dataset=self.valid_dataset, sampler=self.valid_sampler, shuffle=False)
    ...
    def train(self):
        ...
        if self.local_rank == 0:
            ...
            self.logger.info(summary(self.model, input_data=x, verbose=0))
    ...
    def train_epoch(self, epoch, train_loader=None):
        ...
        for metric_func in self.metric_func_list:
            score = metric_func(y_pred_list, y_true_list)
